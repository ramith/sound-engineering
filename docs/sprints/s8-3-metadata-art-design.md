...
**Concurrency (Swift-6):** extraction is CPU/IO-bound + parallelizable; store writes serialize on GRDB's writer (the store is now `final class LibraryStore: Sendable` over a GRDB `DatabaseWriter` — not the actor this doc originally assumed).
