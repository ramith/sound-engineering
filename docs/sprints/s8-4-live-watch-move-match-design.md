...
Albums-before-artists so a dead album's references are gone before the artist reachability check. Returns `(albums:Int, artists:Int, genres:Int)` for logging/verification. **Run it BEFORE `sweepOrphanArtwork`** (deleting an album nulls its `artwork_key`, orphaning art the artwork sweep then reclaims). It is **library-wide reachability** (NOT folder-scoped — a facet used by a track in another folder is kept). Gate it on churn (`orphansSwept>0 || metadataApplied>0`) to avoid three anti-joins on every steady-state pass → **Open Decision D6** (recommend gated). Single-writer actor serializes it against the metadata resolvers; cross-instance it's write-locked by `BEGIN IMMEDIATE` and idempotently recoverable — same posture as the artwork sweep.
