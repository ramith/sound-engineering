...
| **S10.7** 🔨 | Liquid Glass UI polish | 6 | Adopt the founder-approved **8a "Liquid Glass"** design ([docs/design/now-playing-7a/](../design/now-playing-7a/README.md)) on the Now Playing tab + global shell: glass tokens in `DesignSystem`, ambient content glows, analyzer lens (dB grid + peak-hold), hero title/badges, inspector as a trailing 260pt glass column, chrome + footer glass. Native macOS 26 glass APIs first; both appearances; Reduce Transparency/Motion honored. Design: [s10-7-liquid-glass-design.md](s10-7-liquid-glass-design.md). *(polish — not an R1 gate; other tabs adopt the tokens as a fast-follow)* | S10.3 |
...
**Phase 1 total:** ~100 SP (S10 expanded into sub-sprints S10.1–S10.7; S10.5 cut). Realistic to release in two waves (R1 after S10.1–S10.4 + S10.3; S10.6/S10.7 are polish/enhancements; R2 after S14).
...
- **Release R1 ("a real player"):** after **S10.1–S10.4** — daily-driver: library, browse, search, queue, playlists, media keys, bit-perfect playback. *(All R1 gates shipped as of 2026-07-17; the founder chose to land S10.7 Liquid Glass polish before cutting R1.)*
