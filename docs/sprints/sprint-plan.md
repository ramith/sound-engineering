...
| **S9 🟡 NEARLY DONE** | Browse & search UI | 8 | Album-grid; Artist/Album/Genre/Year views; incremental search; cover-art rendering; click-to-queue. **Status (2026-07-09):** shipped & merged (PRs #28–#31) — S9.1 DAO reads + `LibraryTrackDisplay`, S9.2 FTS5 search (schema v2), S9-Q1/Q2 queue/advance core (`PlaybackQueueKit`), S9.4 Albums grid+detail, **S9.5** Songs list + incremental filter (preserves the column sort — native macOS paradigm) + sortable/**customizable** full-catalog columns (`TableColumnCustomization` + `@AppStorage`) + queue-add toast (truthful counts) + row artwork + play-tracking + full VoiceOver a11y + type-to-select. **Remaining = S9.6:** Artists/Genres/Years lists+detail (still `LibraryPlaceholderView`) + drag-to-queue; the A–Z jump rail is deferred (no SwiftUI `Table` scroll-to-row API → rides the `NSTableView` fallback). | S8 |
...
**Status (2026-07-09 — doc reconciliation):** S6 / S7 / **S8** ✅ done; **S9** all slices shipped & merged EXCEPT **S9.6** (Artists/Genres/Years lists+detail). **Next: S9.6 → S10 → Release R1.** (Also merged along the way: a TSan-verifiable wait-free SPSC parameter-snapshot transport — replaced the fence-based seqlock that couldn't be verified under `make tsan` — see [reference in the DSP layer]; and the strict-gate `periphery` hardening.)
...
**Last updated:** 2026-07-09 (status reconciliation: S8 ✅, S9 nearly done → S9.6 next) · **Maintained by:** AdaptiveSound team · **Methodology:** [00-sprint-model.md](00-sprint-model.md) · **Backlog:** [../product/backlog.md](../product/backlog.md)
