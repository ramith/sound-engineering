...
| **S8 🟡 IN PROGRESS** | Library spine: scan + persistent DB | 9 | Folder scan/watch, metadata + embedded-art extraction, incremental rescan, persistent store (GRDB/SQLite), art cache. Headless-testable via CLI/C++ harness before UI. **Status (2026-07-05):** the spine has largely landed on `main` — `LibraryScan` + `LibraryStore` (schema: folders/tracks/artists/albums/genres/artwork/track_genres; S8.4-slice-1 id-preserving move-match in `LibraryStore+MoveMatch.swift`), headless-gated by `swift run VerifyLibraryStore`. Next user-facing step is **S9** browse UI; Gate 1 (playlist filter — see known-issues SEQ-1) stays open until before S10. | S6 (foundation) |
