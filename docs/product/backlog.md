...
- Removing a scan folder root **keeps its tracks as loose** (`tracks.folder_id → NULL` via `ON DELETE SET NULL`, per the shipped S8.1 DDL — **not** `CASCADE`); `removeRoot` then deletes only the now-loose tracks that no playlist references, so **playlist memberships survive** (a curated playlist keeps its songs, playable from their files). It does not touch tracks under other roots or pre-existing loose tracks. *(Erratum, 2026-07-02: this AC previously said `ON DELETE CASCADE`, contradicting the founder-locked keep-as-loose decision and the shipped code — corrected here.)*
