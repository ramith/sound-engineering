...
namespace AdaptiveSound
{
    ...
    void LoudnessModule::initialize(uint32_t sampleRate, uint32_t maxFrames) noexcept
    {
        ...
        measurementThread_ = std::jthread([this](const std::stop_token& stopToken)
                                          { runMeasurementLoop(stopToken); });
    }

    void LoudnessModule::process(const LoudnessParams& params,
    ...
    float LoudnessModule::measuredLufsIntegrated() const noexcept
    {
        return measuredLufsIntegrated_.load(std::memory_order_acquire);
    }
    float LoudnessModule::measuredLufsShortTerm() const noexcept
    {
        return measuredLufsShortTerm_.load(std::memory_order_acquire);
    }
    float LoudnessModule::measuredLufsMomentary() const noexcept
    {
        return measuredLufsMomentary_.load(std::memory_order_acquire);
    }
    float LoudnessModule::currentMakeupGainLinear() const noexcept
    {
        return makeupGainLinear_.load(std::memory_order_acquire);
    }
    uint64_t LoudnessModule::droppedFrameCount() const noexcept
    {
