...
void CrossfeedModule::process(const CrossfeedParams& params, const MultichannelView& block) noexcept
{
    ...
    for (uint32_t frame = 0U; frame < frames; ++frame)
    {
        ...
        const uint32_t readIdxRL =
            (pathRtoL.writeIndex + kMaxCrossfeedDelayFrames - delay) %
        ...
        const uint32_t readIdxLR =
            (pathLtoR.writeIndex + kMaxCrossfeedDelayFrames - delay) %
            static_cast<uint32_t>(kMaxCrossfeedDelayFrames);
