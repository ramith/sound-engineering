...
/// Hearing-safety cumulative-gain clamp for the 31-band EQ (Sprint 4, Milestone 5).
///
...
/// cumulative level.
///
/// Algorithm (per the Sprint 4 spec): sum the signed band gains; if the sum
/// exceeds `cumulativeLimitDb`, scale **every** band by `limit / sum`. A single
/// uniform scale factor preserves the user's intended shape (inter-band ratios)
/// and the sign of every band — boosts stay boosts, cuts stay cuts — while
/// bringing the aggregate boost down to exactly the limit.
///
/// Scope / notes:
...
///   bounded to `[−20, +12]` dB upstream in `EQViewModel`, so the signed sum
...
/// - Confidence-gated stricter clamping (spec §4) is deferred to the future
///   Arbiter / NL control plane, which does not exist yet.
enum EQSafetyClamp {
